...
$data = array(
	 ...
	 */
	'list_action_buttons'    => isset( $list_action_buttons ) && is_array( $list_action_buttons ) ? $list_action_buttons : array(),
	'list_action_popup_btns' => isset( $list_action_popup_btns ) && is_array( $list_action_popup_btns ) ? $list_action_popup_btns : array(),
...
if ( ! in_array( $data['skin'], array( 'default', 'classic' ), true ) && is_mvl_pro() ) {
	...
	<div class="listing-list-loop stm-isotope-listing-item stm-listing-directory-list-loop" data-listing-id="<?php echo esc_attr( $data['listing_id'] ); ?>" data-post-type="<?php echo esc_attr( $data['post_type'] ); ?>">
		...
		<div class="content">
			...
			<?php do_action( 'mvl_rental_inventory_card_actions', $data ); ?>
		</div>
	</div>
	<?php
}
